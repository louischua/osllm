...
class OpenLLMInference:
    ...
    def _load_onnx_model(self):
        ...
        self.onnx_session = ort.InferenceSession(str(self.model_path / "model.onnx"), providers=providers)
    ...
    def generate(
        ...
        if self.detected_format == "onnx":
            return self._generate_onnx(prompt, max_length, temperature, top_k, num_return_sequences, stop_sequences)
        else:
            return self._generate_pytorch(prompt, max_length, temperature, top_k, top_p, num_return_sequences, stop_sequences)
    
    def _generate_pytorch(self, prompt: str, max_length: int, temperature: float, 
                         top_k: Optional[int], top_p: Optional[float], 
                         num_return_sequences: int, stop_sequences: Optional[List[str]]) -> List[str]:
        """Generate using PyTorch model."""
        # Tokenize prompt
        input_ids = self.tokenizer.encode(prompt)
        input_tensor = torch.tensor([input_ids] * num_return_sequences, dtype=torch.long, device=self.device)
        ...
        with torch.no_grad():
            ...
            for _ in range(num_return_sequences):
                ...
                if hasattr(self.model, 'generate'):
                    ...
                    generated_text = self._simple_generate(input_tensor[:1], max_length, temperature)
    ...
    def _generate_onnx(self, prompt: str, max_length: int, temperature: float,
                      top_k: Optional[int], num_return_sequences: int, 
    ...
    def _simple_generate(self, input_tensor: torch.Tensor, max_length: int, temperature: float) -> str:
    ...
    def get_info(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "parameters": self.config.get('n_embd', 0) * self.config.get('n_layer', 0),  # Approximate
