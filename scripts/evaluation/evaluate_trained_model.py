...
def main():
    ...
    for prompt in test_prompts:
        full_text, generated = test_text_generation(
            model, tokenizer, prompt, max_tokens=50, temperature=0.7, top_k=40
        )
        print()
    ...
    print(
        f"   Model quality: {'Good' if avg_perplexity < 50 else 'Needs improvement' if avg_perplexity < 100 else 'Poor'}"
    )
