...
class TestTextGeneration(unittest.TestCase):
    ...
    def test_generation_parameters(self):
        ...
        # Test top_p sampling
        config_top_p = TextGenerationConfig(
            max_new_tokens=10,
            top_p=0.9
        )
        
        # Test top_k sampling
        config_top_k = TextGenerationConfig(
            max_new_tokens=10,
            top_k=50
        )
    ...
    def test_concurrent_requests(self):
        ...
        with patch('inference_server.load_model') as mock_load_model:
            ...
            while not results.empty():
                result = results.get()
                self.assertIn(result, [200, "Error"])  # Should either succeed or handle error gracefully
